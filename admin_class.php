...
Class Action {
	...
	public function __construct() {
		ob_start();
		//Establish database connection so Action{} class can access the database 
   	include 'db_connect.php';
    //Assign value db to $conn in db_connect.php
    $this->db = $conn;
	}
	//Destructor for cleanup activities when an object destroyed
	function __destruct() {
	...
	function login(){
		extract($_POST);// Extract value from the $_POST
			$qry = $this->db->query("SELECT *,concat(lastname,', ',firstname,' ',middlename) as name 
			FROM users where email = '".$email."' and password = '".md5($password)."' ");
		if($qry->num_rows > 0){
			//Iterate through the matching result of the $qry
			...
			return 3; // Login attempt was unsuccessful/user was not found
		}
	}
	function logout(){
		session_destroy();
