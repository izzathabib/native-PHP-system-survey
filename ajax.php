<?php
ob_start();
//Retrieve action value through URL(uses $_GET)
$action = $_GET['action'];
include 'admin_class.php'; //The definition of Action() class are within the admin_class.php
$crud = new Action(); //New object for class Action()
if($action == 'login'){
	$login = $crud->login();
	if($login)
		echo $login;
